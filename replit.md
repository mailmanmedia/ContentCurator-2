...
**Professional Overlay System (Oct 2025)**: True broadcast-quality overlay layer system rendering on top of composited video output (not in grid cells). Overlays configured via dialog with text input, position selector (top/bottom), animation types (scroll/fade/pulse), and three Liverpool FC branded template presets: Breaking News (red #C8102E background, white text, scrolling), Live Updates (navy #002147 background, gold #F6EB61 text, scrolling), and Match Info (gold background, navy text, fade). Overlays render at full canvas width with smooth scrolling animation and support visibility toggle and edit/remove controls. Multiple simultaneous overlays supported with independent configuration. Component: `VideoCompositor.tsx`.

**iOS Camera Detection & Text Customization (Oct 2025)**: Enhanced overlay system with comprehensive text controls and iOS device support. Camera detection properly requests MediaStream permission via getUserMedia() before enumerating devices, enabling support for iPad Pro, iPhone, and Continuity Camera on macOS. Temporary stream is immediately stopped after permission grant. Text customization includes font family selector (League Spartan, Libre Franklin, JetBrains Mono, Arial, Georgia), scroll speed slider (1-100 range scaled to pixel values), and directional scrolling (left/right horizontal, up/down vertical). Canvas font rendering properly quotes multi-word font families for correct display. All controls include testid attributes and integrate with template presets. Components: `LivePresentation.tsx` and `VideoCompositor.tsx`.

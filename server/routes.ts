...
import { registerMetaAgentRoutes } from "./routes/metaAgent";
...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post("/api/admin/clean-player-ids", async (req, res) => {
    try {
      ...
      const playersWithoutId = await db
        ...
        .where(isNull(footballPlayers.player_id));
  ...
  app.get("/api/database/players/top-scorers", async (req, res) => {
    try {
      ...
      const players = await db.select({
        ...
        name: footballPlayers.name,
        photo: footballPlayers.photo,
        goals: playerSeasonStatistics.goalsTotal,
        assists: playerSeasonStatistics.goalsAssists,
        appearances: playerSeasonStatistics.gamesAppearances,
        minutes: playerSeasonStatistics.gamesMinutes,
        rating: playerSeasonStatistics.gamesRating,
        position: footballPlayers.position
      })
      .from(playerSeasonStatistics)
      .innerJoin(footballPlayers, eq(playerSeasonStatistics.playerId, footballPlayers.id))
      ...
      .orderBy(desc(playerSeasonStatistics.goalsTotal))
      .limit(limit);
  ...
  app.get("/api/database/fixtures/upcoming", async (req, res) => {
    try {
      ...
      // Use raw SQL query to bypass Drizzle issues
...
</changes>This commit registers the Meta-Agent routes within the Express application.
<replit_final_file>
import type { Express } from "express";
import { createServer, type Server } from "http";
import { storage } from "./storage";

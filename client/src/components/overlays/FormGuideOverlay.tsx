diff --git a//dev/null b/form-guide-overlay.tsx
index 0000000000000000000000000000000000000000..d42c03f14ac19deb192be9089f2b8b4197114d58 100644
--- a//dev/null
+++ b/form-guide-overlay.tsx
@@ -0,0 +1,1167 @@
+/**
...
+    );
+
+    const streakType = trimmedMatches[0]?.result;
+    let streakLength = 0;
+    if (streakType) {
...
+    const computed = clamp(Math.min(widthScale, heightScale), 0.55, 1.6);
+
...
+  }, [width, height]);
+
+  const circlePx = px(circleSize, { min: 28, max: 88 });
+  const titlePx = px(titleSize, { min: 14, max: 28 });
+  const labelPx = px(labelSize, { min: 10, max: 18 });
+  const spacing = px(layout === "horizontal" ? 18 : 12, { min: 8, max: 28 });
+  const padding = px(18, { min: 12, max: 28 });
...
+          gap: spacing,
+          flex: layout === "horizontal" ? "0 0 auto" : 1,
+        }}
+      >
+        <div
+          style={{
